...
test('uses PR-reported file count when fetched files are capped', () => { const a=scorePullRequest({additions:2,deletions:1,changed_files:50}, [{filename:'src/a.js'}], c); assert.equal(a.filesChanged,50); assert.ok(a.factors.some(x=>x.includes('50 files changed'))); });
